...
def find_profanity(filepath,filename,set_filter_stopwords,badwords):
    ...
    with open('results_' + filename, 'a+', newline='') as outputfile:  # opening required files to read and write
...
def plot():
    ...
    csv_comedy = pd.read_csv('results_comments_comedy_good.csv')
    ...
    ax.set_xlabel("Percent profanity", fontsize=16);
...
if __name__ == "__main__":
    ...
    badwords = get_set(txt_filepath + 'badwords.txt')                         # retreiving set of bad words
